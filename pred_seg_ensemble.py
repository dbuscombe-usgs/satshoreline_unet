...
sample_filenames = [f for f in sample_filenames if 'ir' not in f]
...
for counter,f in enumerate(sample_filenames):
    ...
    nir = seg_labelfile2tensor(f.replace('aug_images', 'aug_nir') )/255
    print(nir.shape)
    ...
    E = (E/len(MODELS)).astype(np.uint8)
    E = np.round(median(E, disk(median_filter_value))).astype(np.uint8)
    ...
    plt.imshow(image[:,:,:3])
    plt.imshow(E, alpha=0.3, cmap=plt.cm.bwr, vmin=1, vmax=nclasses-1)
    plt.axis('off')
    plt.title(name)
    # plt.show()
    plt.savefig(f.replace('.jpg', '_pred_ensemble.png'),
    ...
    plt.title(name)
    # plt.show()
    plt.savefig(f.replace('.jpg', '_nir.png'),
            dpi=300, bbox_inches='tight')
    plt.close('all')
    ...
    plt.title(name)
    # plt.show()
    plt.savefig(f.replace('.jpg', '_im.png'),
            dpi=300, bbox_inches='tight')
    plt.close('all')
    #convert -delay 100 -loop 1 $(ls *pred_ensemble.png | sort -V) sentinel2_pred_demo.gif
